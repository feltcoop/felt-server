		...
		const data = await db.sql<Community[]>`		
			SELECT c.community_id, c.name,
				...
				(
					SELECT array_to_json(coalesce(array_agg(row_to_json(d)), '{}'))
					FROM (
						SELECT p.persona_id, p.name FROM personas p JOIN memberships pc ON p.persona_id=pc.persona_id AND pc.community_id=c.community_id
					) d
				) as membership
			FROM communities c JOIN (
				SELECT DISTINCT pc.community_id FROM personas p JOIN memberships pc ON p.persona_id=pc.persona_id AND p.account_id = ${account_id}
			) apc
