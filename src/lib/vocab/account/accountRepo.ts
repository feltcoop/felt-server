...
import type {Account, AccountModel, CreateAccountParamsType} from '$lib/vocab/account/account.js';
...
export const accountRepo = (db: Database) => ({
	create: async ({
		name,
		password,
	}: CreateAccountParamsType): Promise<Result<{value: Account}, ErrorResponse>> => {
